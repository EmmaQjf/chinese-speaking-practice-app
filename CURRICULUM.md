...
Start with **Phase 2.1** below (design + `Class` model). Build in order; each step reuses patterns from Phase 1 (Mongoose model, Zod, `fetch`, session).
...
|--------|----------------|----------------|-------|
| **2.1** | **Design + `Class` model** — fields e.g. `name`, `joinCode` (unique string), `teacherId` (ObjectId ref to `User`), `timestamps`. Export model like `User` (`models.Class ?? model(...)`). | How to model a *resource* that belongs to a user; `Schema.Types.ObjectId` + `ref: "User"`; why `joinCode` is `unique: true`. | |
| **2.2** | **`ClassMembership` model** (recommended) — `classId`, `userId`, maybe `roleInClass` (`teacher` \| `student`), unique compound index on `(classId, userId)` so a user can’t join twice. *Alternative:* store `memberIds` array on `Class` (simpler at first, harder to query at scale). | Many-to-many: users ↔ classes; indexes for fast lookups and no duplicates. | |
| **2.3** | **Teacher creates class — API only** — `POST /api/classes` (or `app/api/classes/route.ts`). Parse JSON body (`name`). Use **`getServerSession(authOptions)`** to require login; if `session.user.role !== "teacher"`, return 403. Generate a **short random join code** (e.g. 6–8 chars, avoid ambiguous `0`/`O` if you like), ensure uniqueness (retry or loop). Create `Class` with `teacherId: session.user.id`. Optionally **auto-create** a `ClassMembership` row for the teacher so “my classes” queries stay uniform. | Route handlers in App Router; **auth on the server**; separating “who can call this” from “what we save”. | |
